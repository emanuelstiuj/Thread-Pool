...
void enqueue_task(os_threadpool_t *tp, os_task_t *t)
{
	...
	pthread_cond_signal(&(tp->cond_dequeue));
...
os_task_t *dequeue_task(os_threadpool_t *tp)
{
	...
	while (queue_is_empty(tp)) {
		if (!tp->threads_working) {
			pthread_cond_broadcast(&(tp->cond_dequeue));
			pthread_mutex_unlock(&(tp->mutex_queue));

			return NULL;
		}
		pthread_cond_wait(&(tp->cond_dequeue), &(tp->mutex_queue));
	}
	tp->threads_working++;
...
os_threadpool_t *create_threadpool(unsigned int num_threads)
{
	...
	pthread_cond_init(&(tp->cond_dequeue), NULL);
	sem_init(&(tp->sem_dequeue), 0, 0);
